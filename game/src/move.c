...
void ativa_flags(char flags[32], int flag[20]) {
    ...
    for (int i = 0; flags[i]; i++) {
        ...
        switch (flags[i]) {
            ...
            case '>': flag[5] = 1; break;
            case '~': flag[6] = 1; break;
            case 'm': flag[7] = 1; break;
            case 'M': flag[8] = 1; break;
            case 'x': flag[9] = 1; break;
            case 'X': flag[10] = 1; break;
            case 'c': flag[11] = 1; break;
            case 'C': flag[12] = 1; break;
            case 'd': flag[13] = 1; break;
            case 'D': flag[14] = 1; break;
            case 'V': flag[15] = 1; break;
            case 'a': flag[16] = 1; break;
            case 'A': flag[17] = 1; break;
            case 'k': flag[18] = 1; break;
            case 'K': flag[19] = 1; break;
        }
    }
}
...
int verifica_pilha_origem(int flag[20],
    ...
    if (flag[2]){
        for (int i = inicio; i < fim; i++){
            if (origem->pilha[i].valor != origem->pilha[i+1].valor + 1)
                return 0;
        }
    }
    ...
    if (flag[7]){
        for (int i = inicio; i < fim; i++){
            if (origem->pilha[i].naipe != origem->pilha[i+1].naipe)
                return 0;
        }
    }
    ...
    if (flag[11]){
        for (int i = inicio; i < fim; i++){
            if (cor(origem->pilha[i]) != cor(origem->pilha[i+1]))
                return 0;
        }
    }
...
int verifica_pilha_destino(int flag[20],
    ...
    if (flag[15] && destino->tamanho_pilha != 0)
        return 0;

    if (destino->tamanho_pilha == 0){

        if (flag[4] || flag[5] || flag[6] || flag[8]
         || flag[10] || flag[12] || flag[14])
            return 0;
    }
        ...
        CARTAS topo_destino =
            destino->pilha[destino->tamanho_pilha - 1];

        if (flag[4] &&
            topo_origem.valor != topo_destino.valor - 1)
            return 0;
        ...
        if (flag[6] &&
            abs(topo_origem.valor - topo_destino.valor) != 1)
            return 0;

        if (flag[8] &&
            topo_origem.naipe != topo_destino.naipe)
            return 0;
        ...
        if (flag[12] &&
            cor(topo_origem) != cor(topo_destino))
            return 0;
